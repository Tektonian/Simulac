...
from .runtime import RobotRuntime, StuffRuntime

type RuntimeObject = StuffRuntime | RobotRuntime
...
class INativeContext(Protocol):
    ...
    Four layers will expose APIs like below, and abstraction level we become shallow, as the level increasing.
        ...
        Native Escape Hatch
            runner.context("mujoco")
            runner.context("newton")
            runner.context("genesis")
    ...
    ```python
    with runner.context("mujoco") as mj:
